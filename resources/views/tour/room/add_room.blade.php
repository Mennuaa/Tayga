...
@endif
        <form action="{{ route('room.add') }}"  method="POST" enctype="multipart/form-data" >
            ...
            @method('post')
             ...
             <div class="date">
                 <label for="">Оценка (В виде звезд)</label>
                 <input class="form-control input" value="{{ old('rating', $request->rating) }}" name="rating" type="number"  aria-label="default input example">
             </div>
                     </div>
             </div>
                 ...
                 <div class="form-floating">
                     <textarea class=" input form-control" placeholder="Leave a comment here" name="comforts" id="floatingTextarea">
                       {{ old('comforts', $request->comforts) }}
                     </textarea>
                 <label for="floatingTextarea">Удобства в номере</label>
               </div>
               <div class="form-floating">
                <textarea class=" input form-control"  placeholder="Leave a comment here" name="services" id="floatingTextarea">
                    {{ old('services', $request->services) }}
                </textarea>
            <label for="floatingTextarea">Услуги в номере</label>
          </div>
            </div>
             <div class="subtitle">
                <div class="subtitle_info">
                    <label for="">Под заголовок</label>
        <input class="form-control input" value="{{ old('subinfo', $request->subinfo) }}" type="text" name="subinfo"  aria-label="default input example">

                </div>
                <input type="file" name="avatar"   class="myfrm form-control mr-2">
